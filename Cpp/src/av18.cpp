// av18.cpp -- translated from av18.f
//
          ...
          int* iparam)
{
    ...
    // Grid parameters
    constexpr int num_grid = 241;
    constexpr double grid_step = 0.05;
    constexpr double grid_max = (num_grid - 1) * grid_step;

    // Number of cases (S-state and D-state)
    constexpr int num_case = 2;

    static int ls[3] = { 0, 0, 2 };  // 1-based

    // Tabulated wave functions: phis[j][icase], 0-based j index, 1-based icase
    // phis(0:num_grid-1, num_case)
    static real4 phis[3][num_grid] = {
        { 0 },  // [0] unused
        // S-state (icase=1)
        {
    ...
    // Tabulated potentials: vs[icase][j], 0-based j, 1-based icase
    static real4 vs[3][num_grid] = {
        { 0 },  // [0] unused
        // S-state potential (icase=1)
        {
    ...
    static real4 phi_tails[3][3] = {
        { 0 },
        { 0.f, .21114f, .2316f },      // phi_tails(:,1)
        { 0.f, .021156f, .23113f }      // phi_tails(:,2)
    };
    static real4 v_tails[3][3] = {
        { 0 },
        { 0.f, -18.019f, .72772f },     // v_tails(:,1)
        { 0.f, -656.41f, .71085f }       // v_tails(:,2)
    };
    static real4 v_couls[3] = { 0.f, 0.f, 0.f };  // 1-based
    ...
    // =========================================================================
    // Setup (ireque = 1): check for error in l
    // =========================================================================
L100:
    if (l != 0 && l != 2) {
        printf("\n****** error in linkule %.8s\n", alias.data);
        printf(" ******  L =%5d   L must = 0 or 2\n", l);
        numout = 2;
        iretur = -1;
        return;
    }
    ...
    jp = l + 1;
    intger[12] = 0;        // intger(12) = NODES = 0
    // The potential depth is always set to 1.0
    flt[62] = 1.0;         // flt(62) = FITMUL = 1.0
    if (flt[43] == undef) flt[43] = 1.0;   // flt(43) = R0
    if (flt[1] == undef) flt[1] = 0.4;      // flt(1) = A = diffuseness
    if (flt[12] == undef) flt[12] = ebounds[icase]; // flt(12) = E
    jblock[1] = 2.0;      // jblock(1) = J = 2*J = 2
    jblock[8] = 1.0;      // jblock(8) = JSP = 2*SP = 1
    jblock[9] = 1.0;      // jblock(9) = JST = 2*ST = 1
    // The default spec. amplitudes
    if (flt[53] == undef) flt[53] = spams[icase]; // flt(53) = SPAM
    // "spamp" or "spamt", as appropriate, is always set to "spam".
    {
        int i_sw = iswtch[10];  // iswtch(11) = NEXT (Fortran 1-based IS(11); C++ 0-based IS[10] since IS has no +1 offset)
        if (i_sw == 1 || i_sw == 2) flt[53 + i_sw] = flt[53];
    }
    return;
...
L300:
    {
        double am = flt[24];
        double e  = flt[12];
        double rt4pi = cnstnt[2];
        double hbarc = cnstnt[6];
        double alsq  = -2.0 * am * e / (hbarc * hbarc);
        double alpha = std::sqrt(alsq);

        bool wfsw = (ipotyp == 6);

        double rt   = rstart;
        double rend = rstart + (numpts - 1) * stepsz;
        int iend    = (int)((std::min(rend, grid_max) - rstart) / stepsz);
        double hi   = 1.0 / grid_step;
        ...
        for (int i = 1; i <= iend; i++) {
            ...
            if (wfsw) {
                double phi = x0 * phis[icase][j] + xpl * phis[icase][j + 1]
                           + xmn * phis[icase][j - 1];
                array1[i] = rt * phi;
            }

            double v_val = x0 * vs[icase][j] + xpl * vs[icase][j + 1]
                         + xmn * vs[icase][j - 1];
            array2[i] = -v_val;
        ...
        // Add the tails
        int ist = iend + 1;
        for (int i = ist; i <= numpts; i++) {
            if (wfsw) {
                double x  = rt * phi_tails[icase][2];
                double xi = 1.0 / x;
                double f  = std::exp(-x) * xi;
                if (l == 1) f = (1.0 + xi) * f;
                if (l == 2) f = (1.0 + 3.0 * xi + 3.0 * xi * xi) * f;
                array1[i] = rt * phi_tails[icase][1] * f;
            }

            double x     = rt * v_tails[icase][2];
            double v_val = v_tails[icase][1] * std::exp(-x) / x;
            v_val = v_val + v_couls[icase] / rt;
            array2[i] = -v_val;
